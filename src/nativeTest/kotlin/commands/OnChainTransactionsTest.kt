...
class OnChainTransactionsCommandTest {
    @OptIn(ExperimentalCli::class)
    private fun runTest(eclairClient: IEclairClientBuilder): DummyResultWriter {
        val resultWriter = DummyResultWriter()
        val command = OnChainTransactionsCommand(resultWriter, eclairClient)
        val parser = ArgParser("test")
        parser.subcommands(command)
        parser.parse(arrayOf("onchaintransactions", "-p", "password", "--count", "2", "--skip", "0"))
        return resultWriter
    }
    ...
    fun `successful request`() {
        val resultWriter =
            runTest(DummyEclairClient(onchaintransactionsResponse = DummyEclairClient.validOnChainTransactionsResponse))
        ...
        assertEquals(
            format.parseToJsonElement(DummyEclairClient.validOnChainTransactionsResponse),
    ...
    fun `serialization error`() {
        val resultWriter = runTest(DummyEclairClient(onchaintransactionsResponse = "{invalidJson}"))
        assertNull(resultWriter.lastResult)
